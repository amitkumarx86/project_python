...
class ImageProcessor(QMainWindow):
    ...
    global image, l1, grayScaleBtn, gaussianBlurBtn, cannyBtn, hughPBtn, hughBtn, sobelXBtn, sobelYBtn, laplacianBtn,\
    simpleThresBtn, adaptiveThresGaussianBtn, otsuThresBtn, adaptiveMeanThresBtn, undoBtn
    ...
    def toggleOperatorButtons(self,boolean):
        for button in [self.grayScaleBtn, self.gaussianBlurBtn, self.cannyBtn, self.hughPBtn, self.hughBtn, self.sobelXBtn, \
    ...
    def hughFun(self):
        if(len(self.image.shape) > 2):
            ...
            temp_image=copy.copy(self.image)
            self.imageStack.append(temp_image)
            self.buttonStack.append(self.hughBtn)
    ...
    def hughPFun(self):   
        if(len(self.image.shape) > 2):
            ...
            temp_image=copy.copy(self.image)
            self.imageStack.append(temp_image)
            self.buttonStack.append(self.hughPBtn)
